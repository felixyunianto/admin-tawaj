                    ...
                    <form action="{{ route('highlight.update', $highlight->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input name="title" class="form-control" type="text" value="{{$highlight->title}}"
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description" id="" cols="30" rows="10"
                                    placeholder="Isikan deskripsi highlight">{{$highlight->description}}</textarea>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="link_type" id="link_type" class="form-control">
                                        <option value="article" @if($highlight->link_type == 'article') selected @endif>Artikel</option>
                                        <option value="content" @if($highlight->link_type == 'content') selected @endif>Content</option>
                                        <option value="button_page" @if($highlight->link_type == 'button_page') selected @endif>Button Page</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-8">
                        ...
                        <div class="col-md-12 pt-2 pb-2">
                            ...
                            <img src="{{$highlight->images}}" alt="" width="300" height="150">
                        </div>
                        <div class="col-md-12">
        ...
        if(valueLinkType === 'button_page'){
            ...
            <select name="link" class="form-control">
                ...
                @foreach ($button_pages as $item)
                    <option value="{{$item->id}}" @if($item->id == $highlight->link  && $highlight->link_type == 'button_page') selected @endif>{{$item->title}}</option>
                    @if (count($item->children) > 0)
                        <optgroup label="&nbsp;&nbsp;&nbsp;&nbsp;Child">
                    @endif
                        @foreach ($item->children as $child)
                            <option value="{{$child->id}}" @if($child->id == $highlight->link  && $highlight->link_type == 'button_page') selected @endif>{{$child->title}}</option>
                            @if (count($child->children) > 0)
                                <optgroup label="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sub Child">
                            @endif
                                @foreach ($child->children as $subchild)
                                    <option value="{{$subchild->id}}" @if($subchild->id == $highlight->link  && $highlight->link_type == 'button_page') selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;{{$subchild->title}}</option>
                                @endforeach
                            @if (count($child->children) > 0)
                                </optgroup>
                            @endif
            ...
            <select name="link" class="form-control">
                ...
                @foreach ($contents as $content)
                    <option value="{{$content->id}}" @if($content->id == $highlight->link && $highlight->link_type == 'content') selected @endif>{{$content->title_arab}} <span>({{$item->title_indo}})</span></option>
                @endforeach
            </select>
            `
        }else{
            html = `<input type="text" class="form-control" name="link" placeholder="isikan link" value="{{$highlight->link}}">`
        }
